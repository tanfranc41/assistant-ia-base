...
- Endpoint de test d'authentification **temporaire et non sécurisé** : `GET /api/auth/test` (protégé uniquement par le middleware existant) renvoie `{ "status": "ok", "auth": "passed", "phase": "phase-2" }` quand le header contient le jeton exact `test-token`.
...
- Accepte uniquement le jeton statique `test-token` (pas de déchiffrement, pas de JWT, pas de secret).
...
- Ce mécanisme est **temporaire**, **non sécurisé** et ne doit servir qu'à valider manuellement le passage dans le middleware (ex. `curl -H "Authorization: Bearer test-token" http://localhost:3000/api/auth/test` → 200).
