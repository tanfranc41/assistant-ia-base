...
- Backend minimal (`backend/`) : serveur Express sans authentification ni base de données, sans persistance ni logique métier, avec les endpoints `GET /health` (`{ status: "ok", phase: "phase-2" }`) et `GET /api/info` (`{ name: "AI Assistant", phase: "phase-2", status: "backend baseline active" }`).
